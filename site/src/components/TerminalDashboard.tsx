...
export function TerminalDashboard() {
  ...
  useEffect(() => {
    const fit = () => { if (!box.current || !frame.current) return; const k = Math.max(.6, Math.min(1, box.current.clientWidth / 1440)); frame.current.style.transform = `scale(${k})`; box.current.style.height = Math.round(Math.min(900 * k, 620)) + 'px'; };
    fit(); const ro = new ResizeObserver(fit); if (box.current) ro.observe(box.current); return () => ro.disconnect();
  }, []);
  return (
    <div className="relative mt-8 overflow-hidden pb-[var(--spacing-sec)] pl-[max(40px,calc((100%-1280px)/2+40px))] max-[780px]:px-[28px] max-[600px]:px-5">
      <div ref={box} className="relative h-[620px] w-[calc(100%+56px)] max-w-[1400px] overflow-hidden rounded-[16px] border border-tline bg-tbg shadow-[inset_0_1px_0_rgba(255,255,255,.05),0_30px_80px_-20px_rgba(0,0,0,.7),0_10px_24px_-12px_rgba(0,0,0,.5),0_0_0_1px_rgba(59,130,246,.06)] max-[780px]:w-full">
        ...
        <div className="pointer-events-none absolute inset-y-0 right-0 w-[120px] bg-gradient-to-r from-transparent to-[rgba(11,12,15,.55)] max-[780px]:hidden" />
      </div>
    </div>
  );
}

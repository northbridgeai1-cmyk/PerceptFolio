...
3. In PerceptFolio, open **Settings** and paste the key. It will test itself.
...
1. Create a new repo on GitHub (e.g. `perceptfolio`).
...
4. Your site is live at `https://YOURNAME.github.io/perceptfolio/` within a couple of minutes.
...
A Business account's own login doesn't hold personal investments in PerceptFolio — **Portfolio**, **Watchlist**, and **Alerts** are hidden on your own view (they come back the moment you Switch into a client, since that's their money, not yours), and logging in lands you on Clients instead of Dashboard.

**Fee % per client:** each client row has an editable **Fee %** field plus an "Est. fee owed" column (fee % × that client's unrealized gain, $0 on a loss). This is a private note stored only in your browser, not a billing or payment feature — PerceptFolio has no payment processing and never moves money. See the legal note below before you put a real number in that field.

**What a "client account" actually is here, and why:** in real practice, an adviser or quant who trades for someone else doesn't hold that person's money themselves — the client's actual assets sit at a real brokerage/custodian (Schwab, Fidelity, IBKR, etc.) in the client's name, the adviser gets trading authority over it, and the client's official statements come from that custodian, not from the adviser. On top of that, advisers commonly run their own separate reporting layer (tools like Orion, Black Diamond, Addepar) that mirrors the custodian data into client-facing reports — value, performance, allocation, fees — without being the system of record. PerceptFolio's Clients tab is built to play that second role only: a private tracking/scoring copy, not a real account. That's why a client here doesn't need a real login at all (see the note on the Create Client form) — the email field is just an internal ID unless you choose to hand it to them.

**Client Report:** click **Report** on any client row for a read-only, statement-style summary — total value, unrealized gain ($ and %), allocation by holding type, top 5 holdings, and your fee — timestamped "as of" today. It's a snapshot generated from whatever you've entered by hand, not a live pull from anywhere; useful as a quick client-facing summary, but it's only as current as your last manual update.
...
A "money made over time" chart on the Dashboard, plotting your cumulative gain/loss (and total value, dashed) day by day. Honest limitation: PerceptFolio has no record of your portfolio before you started using it — this starts tracking from today and fills in as you use the app (each Refresh Data, or any manual price/cash edit, logs a snapshot for that day). It is not a substitute for your brokerage's actual historical performance report.
...
1. Go to your repo on github.com (e.g. `github.com/YOURNAME/perceptfolio`).
...
- **The per-client Fee % / "Est. fee owed" field is a local note, not a billing system** — no payment processing exists anywhere in this app. It also doesn't make a performance-fee arrangement legal; see the Rule 205-3 caveat in "Business accounts and Clients" above.
